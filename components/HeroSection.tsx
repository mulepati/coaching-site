    ...
    <section className="panel overflow-hidden px-6 py-10 md:px-8 md:py-12">
      <div className="grid gap-8 md:grid-cols-[1.2fr_0.8fr] md:items-center">
        <div className="space-y-5">
          ...
          <div className="max-w-2xl space-y-3 text-lg leading-8 text-ink/76">
            <p>{description}</p>
            {supportingLine ? <p>{supportingLine}</p> : null}
          </div>
          {cta ? <CTAButton href={cta.href} label={cta.label} /> : null}
        </div>
        <div className="relative min-h-64 rounded-[2rem] border border-ink/10 bg-paper/30 p-5">
          <div className="absolute inset-5 rounded-[1.5rem] border border-dashed border-primary/20" />
          <div className="absolute inset-x-10 top-8 h-16 rounded-full bg-primary/14 blur-2xl" />
          <div className="absolute bottom-8 left-8 h-20 w-20 rounded-full bg-secondary/18 blur-2xl" />
          <div className="relative flex h-full min-h-52 flex-col justify-between rounded-[1.5rem] bg-paper/52 p-6">
